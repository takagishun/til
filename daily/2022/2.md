...
WCAG2 系までは Web Contents Accessibility Guidelines だったが、WCAG3.0 は W3C Accessibility Guidelines 3.0 になった。
ウェブコンテンツ以外の幅広い範囲を想定される。

日本語版 WCA 3.0 概要
https://docs.google.com/presentation/d/1yP_8JGcRLwghdApymX1Yj6SuFudLaybaeAjvRRuDZWg/edit#slide=id.p

WCAG3.0 Project Plan
https://github.com/w3c/silver/wiki  
これみると Recommendation が 2026 年になっている。。まだまだ先か。。
ただプロジェクトの割と初動から追ってるのは価値があるかも!
...
silver プロジェクトの community サイト見つけた

https://www.w3.org/community/silver/draft-final-report-of-silver/  
Silver Design Sprint のレポートがみれる。Silver Design Sprint は wcag3.0 を作成するための調査や議論をする場。 専門家 30 名が召集され、2018/3/18.19 で行われた。

https://www.w3.org/WAI/GL/task-forces/silver/wiki/Design_Sprint_Participants  
Silver Design Sprint の参加者

Silver Design Sprint レポートより

> Silver の構造に関連する研究は、2018 年 3 月までに完了しました。

その成果物 → https://docs.google.com/presentation/d/1POs7orJ4ALB0bq5_vyo4v8RxDcr-5ctwD1noVgpXuJc/edit#slide=id.gc6f73a04f_0_0

> Silver のコンテンツに関連する研究は、今後も継続的に行われる予定です。

https://www.w3.org/WAI/GL/task-forces/silver/wiki/Design_Plan_for_Silver  
Silver の計画書見つけた
...
WCAG3.0 のプロジェクトリポジトリ。issue とか PR 見ればどういう方向に向かっているのかわかる。 ガイドラインの更新作業中のものはhttps://w3c.github.io/silver/guidelines/
にホストされている
...
2 系の達成基準は 1 つでも基準に満たなかったらガイドラインに準拠してないことになるが、3.0 では障害者が利用できる程度なら必ずしも 100%達成する必要がなくなった。
目的が「準拠すること」ではなく、「アクセシブルにする」ことに意識を向けることができそうなので良い方向だと感じた。

https://www.w3.org/TR/wcag-3.0-requirements/  
「WCAG3.0 のための要件」という資料を見つけた。
...
のガイドラインに関連して、 wai-aria-practices のモーダルダイアログのセクション読みながら、実装試してみた。  
https://www.w3.org/TR/wai-aria-practices-1.2/#dialog_modal

- dialog role をつける
- モーダルダイアログの場合は aria-modal: true をつける
  - これを設定することで、スクリーンリーダーなどの支援技術にモダール以外が非活性であることを使える
  - タブ操作ではモーダル外にフォーカスが移動しちゃうので別途対応必要。dialog タグで対応可能っぽいがブラウザ対応状況が悪い
- ダイアログとその title を aria-labelledby で紐付ける
- [option]どの要素が主要な部分かを示すために aria-describedby をつける。しかしコンテンツが複雑な構造になっている場合は省略することが望ましい。

フォーカス制御について以下の要件が必要だが、なかなか良い実装方法が思いつかない。

- モーダルを開くとモーダル内の要素にフォーカスする
- モーダルを閉じるまではモーダル内でフォーカスをループさせる

と思ってたら良さげな記事見つけた https://zenn.dev/dqn/articles/36045bb89d5d69
...
- roles, states, properties
  - aria のロールは要素の種類であり、時間やユーザーの操作によって変化することはない。
  - 状態とプロパティは、要素の重要な属性を示し動的に変更される。これらの属性は，動的に変更される場合でも，利用者エージェント及びオペレーティングシステムが要素を適切に取り扱うことを可能にする
...
- WAI-ARIA を使用する時の原則
  ...
  1. ARIA ロールを使用することは、定義された期待する動作を満たすことを約束すること。
  2. ARIA は既存の HTML ロールを隠蔽したり、書き換えたりできる強力な力を持っている。故に危険でもある。

- ブラウザと支援技術と wai-aria-practices

  - wai-aria-practices にのっているデザインパターンやコード例はあくまで aria を適切に使用するためのものであるので、実際にターゲットとなるブラウザや支援技術において正しく動作するかは徹底的にテストする必要がある

- スマートデバイス
  - aria の機能の中には、モバイルブラウザでサポートされていないものがある

https://www.w3.org/TR/wai-aria-practices-1.2/#aria_ex  
これ以降のセクションが実装例が載っていて面白い
...
.box {
  ...
  animation: bound 3s linear 1s infinite normal none running;
}
...
WAI とは、 Web Accessibility Initiative の略で、W3C のイニシアチブ。世界中の組織と協力して、Web のアクセシビリティを追求している。

WAI のドキュメント
https://www.w3.org/WAI/about/

WAI の中でさらにいくつかの WG に分かれている。

- Accessibility Guidelines Working Group (AG WG)
- Accessible Rich Internet Applications (ARIA) Working Group

※ https://www.w3.org/groups/wg/ Working Group とは一つの仕様を策定するための作業グループで、W3C の会員、諮問委員会の代表が選出した人、招聘されたエキスパート(Invited Experts)
、W3C のスタッフで構成される
...
- wai-aria は HTML5 で提供されているセマンティクスを補強することで動的なコンテンツを持つ web アプリケーションをアクセシブルにするためのもの。
- 仕様書のなかで「適合」セクションがあるが、WCAG とは違いこれはアプリケーション開発者ではなく、ユーザーエージェントが実装すべき要件になる。
- アプリケーション開発者のための wai-area の使い方は「WAI-ARIA Authoring Practices 1.2」というドキュメントにまとめられている

### 仕様書

オリジナル: https://www.w3.org/TR/wai-aria-1.2/
日本語：https://momdo.github.io/wai-aria-1.2/#conformance
...
今日は同僚からペアワイズ法について教えてもらった。ペアワイズ法とは２つの因子の組み合わせを網羅するテスト手法。4 つの因子と、その因子の取りうる値が 3 つずつあったとすると、複合条件テスト全網羅するために 52
通りを行わないと行けないが、ペアワイズ法だと数通りのテストで済む。 ペアワイズ法では 76%程度のバグを潰せるという。後 2 割のバグはペアワイズ法では潰せないので、強化テストをする。  
https://github.com/Microsoft/pict/blob/main/doc/pict.md  
ペアワイズ法を利用したテストケースを出力できる pict というツールを初めて試したが、非常に使いやすい。

## アクセシビリティ(WAI-ARIA)

これまで WCAG を読んできたが、どこからか WAI-ARIA の文章に行き着いた。どうやら WAI-ARIA とは Web コンテンツやアプリケーションのアクセシビリティの関心事に特化したものらしい。 2017
年に勧告になったバージョン 1.1 が最新だが、1.2 も勧告候補になっている

# 2 月 9 日

## アクセシビリティ(wcag2.1)

wcag2.1 の達成基準の達成方法をいくつか眺めていたら、達成方法として例に挙げられているのが HTML4 で書かれたものであることを知った。この達成方法を愚直に真似すると HTML5
に対応してないことでレガシーなコードになる可能性があるな気になった wcag 達成基準を満たす方法は必ずしもその中で紹介されているものに限らないと書いてあるけども。

メインコンテンツが main や h1 でマークアップされていれば、スクリーン・リーダーなどで本文の先頭に簡単に移動することができる。  
...
- deployment
  ...
  - エンボイ: DNS。proxy。SS の IP によしなにリクエスト飛ばす。BFF の pods の側におきたいので同じデプロイメントで収めるなど
- service
  - pod たちの集まり
- ingress
  - service のロードバランサー。外部 port とのつなぎこみ

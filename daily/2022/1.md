...
WCAG3.0のテストやスコアリングについて調べた。
- WCAG3.0にはAtomic testsとHolistic testsの2つのテスト種別がある。 
- Atomic testsはこれまでのテスト同様、通常はコードのテストであり、ブロンズレベルに到達するために使用される。
- Holistic testsは障害者が使用するハードウェアやソフトウェア（支援技術）を使ったユーザビリティテストやマニュアルテスト。これらのテストは、シルバーレベルまたはゴールドレベルに到達するために使用される。
- Atomicテストに合格しても全ての障害者に適するものになっているとは言えないので、そのギャップを埋めるためにHolisticテストがある
- テストの対象はビューとプロセス。ビューは表示、プロセスは一定の操作に対するテスト。プロセスには複数のビューが含まれる
- アトミックテストには、自動評価、半自動評価、人間による評価の組み合わせが含まれうる。
- メソッドの個々テストはtrue/falseか%評価がある。それを平均化したりしてoutcomeのスコアに反映させる

...
調べたのでメモ。
長いformの上部にエラーバーを表示する場合、submitボタンからエラーバーが見える位置までスクロールしたい.
reactのref & useEffectとscroll-into-viewというライブラリを使えばできる
https://www.npmjs.com/package/scroll-into-view

Web APIのElement.scrollIntoView()があるのでサポートブラウザ次第で使える(safariとIEが対応してない)

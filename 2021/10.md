...
文字数判定するのに基本的にはlengthで問題ないが、絵文字とかを許容するとlengthではダメなのを知った。
...
ちなみにWeb API のFileオブジェクトのtypeは拡張子で判別している。
...
またfetch APIでcontent-typeを明記しなかった場合、自動で付与されるが、ここでも拡張子で判別しているらしい(ドキュメントは見つからないが、chrome dev toolで確認できた)
...
npmパッケージではfile-typeなどが、マジックナンバーでのファイル種別判定を採用している
...
拡張子をtxtに変えてみても、ちゃんとpngとして判定されう
...
Linuxのfileコマンドでもマジックナンバーの判定が行われる
```
$ file ./test.txt 
...
React公式ドキュメントの「コード分割」のページを今まで読んだことなかったので、読んでみる。
...
1. import() 構文を使う
2. さらにReact.lazy 関数を使用すると、動的インポートを通常のコンポーネントとしてレンダーすることができる

...
google maps apiのエラーハンドリングでハマったのでメモ。

reactでgoogle mapを扱うことがあったので↓のライブラリを使った。  
https://github.com/JustFly1984/react-google-maps-api/tree/master/packages/react-google-maps-api  
このライブラリはgoogle mapsをロードする際に内部で↓を使っている。  
これはgoogle本家が出していて、これまでhtmlにscriptタグの書く必要があったが、javascriptでscriptタグを動的に読み込めるようにしたもの。  
...
READMEにも記載あるとおり、↓の感じで簡単にロードでき、ロード後にやりたい処理やエラーハンドラを実装できる。
...
`loader.load()` でresolveされるとgoogle.maps apiが操作できるようになる。  
rejectされるとcatch側でエラーハンドリングする。
内部でscriptタグのonerrorで捕捉してエラーをrejectしている。なのでここで捕捉できるのはscriptタグのonerrorで捕捉できるもの、つまり、サーバーがダウンしてたり、srcのurlが間違っていたりするケース。  
https://github.com/googlemaps/js-api-loader/blob/346f8210596357b047c8d6b307189561ff8dfe13/src/index.ts#L420  
なので、invalidなapi keyを渡した場合や、ホワイトリストに登録したドメイン以外からのアクセスした場合などのマップエラーはここで捕捉できない。  
https://developers.google.com/maps/documentation/javascript/error-messages  
ドキュメントを読むとエラーはjavascriptコンソールに表示されるので確認してねと書いてあるが、プログラムから補足するにはgm_authFailure()というグローバル関数を定義して補足できるとある。  
...
ログイン機能があるサービスでユーザーにパスワードを入力してもらってDBなどに保存する時は、セキュリティ上ハッシュ化する。  
セキュアなハッシュ関数としてはsha256が有名。MD5は脆弱性があるのでセキュアなものには使っちゃダメ。  
nodejsでは'crypto'という標準ライブラリがある。    
...
今回、1つのサービス独自の認証機能を作って学んだが、認証/認可機能を他サービスに提供する仕組みもある。    

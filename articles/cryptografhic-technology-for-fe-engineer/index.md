...
より具体的にセキュアな通信についてどのようなものか、そして何が必要そうかイメージしてみましょう。 イメージしやすいように、オフラインでの安全な文通について考えてみます。

小学生のアリスとボブは文通をしており、文通の内容をお互いの親や知り合いにはわからないようにしたいと思っています。そこで、文章を暗号化するようにしました。暗号化の方法はいくつか思いつき、一つは文字を数文字シフトさせ暗号化する方法です。3つずらす場合は「あ」は暗号文では「え」になり、「い」は「お」になります。もう一つは換字表を使い「あ」に対応する文字は「さ」、「い」に対応する文字は「す」という具合に変換して暗号文を作る方法です。
暗号文を復号する場合は同じ分だけシフトしたり、同じ換字表を使い平文に戻します。それぞれの極秘情報はシフト方式の場合は何個ずらすかであり、換字式の場合は換字表です。アリスとボブはどの暗号方式でいくか暗号化してない文通で相談し、「換字式」で行くことにしました。次に極秘情報をどのように共有するかを暗号化してない文通で相談しました。ただ極秘情報である換字表はばれたりすり替えられたりすると大変なので、受け渡しは郵送ではなく直接渡すことになりました。極秘情報を共有できたことで晴れて二人だけしか知らない暗号文で文通ができます。しかし、まだ問題があります。暗号文自体が誰かに改竄されると復号したときに違う文章になってしまうので,
お互いが書いた暗号文ということしっかしわかるようにしたいと思いました。そのためワープロではなく手書きにし、サインを添えることで、筆跡で判断できるようにしました。
...
TLSで通信する場合でも大まかな手順は似ています。 ただしオンラインで完結でき、さらにより安全性が高い暗号化方式が必要です。
実際の通信で暗号化/復号に使う極秘情報の共有を行うときは、それ自体の通信は安全ではないので盗聴されても良い情報だけのやりとりでなんとか極秘情報を共通する仕組みが必要なわけです。

それではTLSのプロトコルを理解しながら、セキュアな通信がどのように確立するのか見ていきたいと思います
